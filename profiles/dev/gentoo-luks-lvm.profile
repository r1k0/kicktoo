...
if [ "${arch}" == "x86" ]; then
    wget -q http://gentoo.mirrors.ovh.net/gentoo-distfiles/releases/"${arch}"/autobuilds/latest-stage3-"$(uname -m)".txt -O /tmp/stage3.version
elif [ "${arch}" == "amd64" ]; then
    wget -q http://gentoo.mirrors.ovh.net/gentoo-distfiles/releases/"${arch}"/autobuilds/latest-stage3-"${arch}".txt -O /tmp/stage3.version
fi
latest_stage_version=$(grep tar.bz2 /tmp/stage3.version)

stage_uri               http://gentoo.mirrors.ovh.net/gentoo-distfiles/releases/"${arch}"/autobuilds/"${latest_stage_version}"
